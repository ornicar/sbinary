...
trait StandardPrimitives extends CoreProtocol {
  ...
  private def readUnsignedLong(in : Input) = (in.readByte & 0xFF).toLong
  ...
  implicit object LongFormat extends Format[Long]{
    def reads(in : Input) = (
      (readUnsignedLong(in) << 56) +
      (readUnsignedLong(in) << 48) +
      (readUnsignedLong(in) << 40) +
      (readUnsignedLong(in) << 32) +
      (readUnsignedLong(in) << 24) +
      (readUnsignedLong(in) << 16) +
      (readUnsignedLong(in) <<  8) +
      (readUnsignedLong(in)      )
    )
...
trait JavaUTF extends CoreProtocol {
  private[this] def readUnsignedByte(in : Input) : Int =
    in.readByte.toInt & 0xFF
  private[this] def readUnsignedShort(in : Input) : Int = 
    (readUnsignedByte(in) << 8) + readUnsignedByte(in)

  private[this] val charBuffer = new ThreadLocal[Array[Char]] { override def initialValue() = new Array[Char](80) }
  private[this] val byteBuffer = new ThreadLocal[Array[Byte]] { override def initialValue() = new Array[Byte](80) }

  private[this] def growBuffers(size: Int) = {
    charBuffer set new Array[Char](size)
    byteBuffer set new Array[Byte](size)
  }
  private[this] def fetchCharBuffer(size: Int) = {
    val buf = charBuffer.get()
    if (buf.length < size) {
      growBuffers(size * 2)
      charBuffer.get()
    }
  ...
  private[this] def fetchByteBuffer(size: Int) = {
    val buf = byteBuffer.get()
    if (buf.length < size) {
      growBuffers(size * 2)
      byteBuffer.get()
    }
  ...
  implicit object StringFormat extends Format[String] {
    private def malformed(index: Int) = sys.error("Malformed input around byte " + index)
    private def partial               = sys.error("Malformed input: Partial character at end")

    def reads(in : Input) = {
      val utflen  = readUnsignedShort(in)
      val cbuffer = fetchCharBuffer(utflen)
      val bbuffer = fetchByteBuffer(utflen)
      ...
      while (count < utflen) {
        ...
        cbuffer(charCount) = (((c >> 4): @switch) match {
    ...
    def writes(out : Output, value : String) {
      ...
      val bbuffer = fetchByteBuffer(utflen + 2)
      var count = 0
