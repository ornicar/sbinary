...
trait Generic extends CoreProtocol {
  ...
  final case class Summand[T](clazz: Class[_], format: Format[T]) { }
  implicit def classToSummand[T](clazz : Class[T])(implicit bin : Format[T]) : Summand[T] =
    Summand[T](clazz, bin)
  implicit def formatToSummand[T](format : Format[T])(implicit mf : ClassManifest[T]) : Summand[T] =
    Summand[T](mf.erasure, format)
  // This is a bit gross.
  implicit def anyToSummand[T](t : T) =
    Summand[T](t.asInstanceOf[AnyRef].getClass, asSingleton(t))
